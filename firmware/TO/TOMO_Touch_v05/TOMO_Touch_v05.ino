/*
   ...
   _______  _______ 
  |       ||       |  Arduino based Touch Synthesizer
  |_     _||   _   |   
    |   |  |  | |  |  TOUCH FIRMWARE v0.5
    |   |  |  |_|  |
    |   |  |       |
    |___|  |_______|   Based on:
   __   __  _______ 
  |  |_|  ||       |   Illutron take on Disney Touch style capacitive touch sensor
  |       ||   _   |   using only passives and Arduino  Dzl 2012
  |       ||  | |  |
  |       ||  |_|  |   Developed by:
  | ||_|| ||       |
  |_|   |_||_______|   FabLabBCN && clemniem 2014
  
  Sweep Capacitances Features:
                      
  1. Store & Calculate all Values on the Arduino
  2. Enhance Resolution through comparing Arrays rather than Maxima 
